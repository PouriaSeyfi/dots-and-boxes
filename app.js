...
function initializeGame() {
    ...
    squares = Array(ROWS).fill(Array(COLUMNS).fill("")).map((r) => {
        return [...r]
    })
...
function showSquareLabel(i, j) {
    ...
    squreLabel.textContent = TURN == PLAYER_ONE ? PLAYER_ONE[0].toUpperCase() : PLAYER_TWO[0].toUpperCase()
...
function createBoardGame() {
    ...
    document.getElementById('player-one-score').innerText = PLAYER_ONE + ' : ' + 0
    document.getElementById('player-two-score').innerText = PLAYER_TWO + ' : ' + 0
}
...
function addScore(turn) {
    if (turn == PLAYER_ONE) {
        R_SCORE++
        document.getElementById('player-one-score').innerText = PLAYER_ONE + ' : ' + R_SCORE
    } else {
        B_SCORE++
        document.getElementById('player-two-score').innerText = PLAYER_TWO + ' : ' + B_SCORE
    }
